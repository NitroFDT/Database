...
-- 1
-- Display all customer data where customer name’s length is at least 10 characters.
...
-- 2
-- Display CustomerName and Day of Transaction (obtained from the day of their transaction).
-- (datename, weekday)
...
-- 3
-- Display Total Transaction (obtained from the total of staff who served transaction before March 2020) (month, year)
...
-- 4
-- Display Month (obtained from month name of transaction date), Total Barbells Sold (by adding all the barbells sold) and Average Barbells Sold 
-- (by showing the average of barbells sold for every transaction) for transactions done in February 2020. Then, combine it with displaying Month
-- (obtained from month name of transaction date), Total Barbells Sold (by adding all the barbells sold) and Average Barbells Sold 
-- (by showing the average of barbells sold for every transaction) for transactions done in March 2020.
-- (union, month, year)
...
--5
-- Display Customer Initial (obtained from first character of customer name) and 
-- Total (obtained from total of the barbell -- bought and adding ‘ Barbell(s)’ in the end) 
-- for every Male -- Customers who bought barbell with weight 10 or 20.
-- (substring, cast) 
...
--6
-- Display CustomerName (obtained from customer last name) and Money Spent 
-- (obtained from sum of barbell quantity multiplied with barbell price)
-- for customer who spent money more than the average of money spent.
...
-- 7
-- Create a view named ‘ViewCustomerInformation’ by showing CustomerName in capital letters, 
-- CustomerGender, CustomerPhone (obtained by replacing the first two number ‘08’ with ‘+62’), 
-- and CustomerAddress (upper, stuff)
...
-- 8
-- Create a view named ‘ViewBarbellBrandTotalWeight’ to show BarbellBrand and Total Weight
-- (obtained from the total of weight according to the brand by adding ‘ kg(s)’ in the end)
-- for all barbell who weight more than 10 kg(s). Must be ordered by descending (from the heaviest)
-- (sum, cast, order by)
...
-- 9
-- Add ‘StaffAddress’ column to MsStaff table with type varchar(100) and 
-- add constraint that the address must have the word ‘Street’. (alter table, add, add constraint)
...
-- 10
-- Update all staff salary by adding it with 10% of its original value. (update)
